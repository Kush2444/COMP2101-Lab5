...
function error-message {
    ...
    echo "${progname}: ${1:-Unknown Error - a moose bit my sister once...}" >&2
}
...
Options:
  ...
  --host           Show hostname information only
  --domain         Show domain name information only
  --ipconfig       Show IP configuration only
  --os             Show operating system information only
  --cpu            Show CPU information only
  --memory         Show memory information only
  --disk           Show disk usage only
  --printer        Show printer information only

If no options are specified, the full report will be shown.
EOF
}
...
# Trap Ctrl+C (SIGINT) to run cleanup and exit gracefully
trap 'cleanup; echo ""; error-exit "Script interrupted by user (Ctrl+C)";' INT
